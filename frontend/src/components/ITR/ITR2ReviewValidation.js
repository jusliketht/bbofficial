// =====================================================
// ITR-2 REVIEW & VALIDATION COMPONENT - COMPREHENSIVE VALIDATION
// Enterprise-grade ITR-2 validation with AI assistance
// =====================================================
...
const ITR2ReviewValidation = ({ 
  ...
  const { totalIncome, totalCapitalGains, totalForeignIncome, totalDeductions, taxableIncome } = calculateTotals();
    ...
    <div className="space-y-6">
      ...
      <div className="dashboard-card-burnblack p-4">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-burnblack-black">Review & Validation (ITR-2)</h2>
            <p className="text-sm text-neutral-500">
              Final review before filing your ITR-2
            </p>
          </div>
        </div>
          ...
          <div className="px-4 pb-4 space-y-4">
            ...
            <div className="border-t border-neutral-200 pt-3">
              <div className="flex justify-between">
                <span className="text-lg font-semibold text-burnblack-black">Total Regular Income</span>
      ...
      {totalCapitalGains > 0 && (
        <div className="dashboard-card-burnblack">
          {renderSectionHeader('Capital Gains Summary', TrendingUp, 'capitalGains', 
            Object.keys(filingData.income.capitalGains || {}).length)}
          
          {expandedSections.capitalGains && (
            <div className="px-4 pb-4 space-y-4">
              {Object.entries(filingData.income.capitalGains || {}).map(([key, value]) => {
                ...
                switch (key) {
                  case 'equity':
                    capitalGainsAmount = (value.stcg || 0) + (value.ltcg || 0) - (value.stcl || 0) - (value.ltcl || 0);
                    assetName = 'Equity Shares';
                    break;
                  case 'debt':
                    capitalGainsAmount = (value.stcg || 0) + (value.ltcg || 0) - (value.stcl || 0) - (value.ltcl || 0);
                    assetName = 'Debt Securities';
                    break;
                  case 'realEstate':
                    capitalGainsAmount = (value.salePrice || 0) - (value.indexedCost || 0);
                ...
                if (capitalGainsAmount === 0) return null;
                
                return (
                  <div key={key} className="p-3 bg-neutral-50 rounded-lg">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-neutral-600">{assetName}</span>
                      <span className="text-sm font-semibold text-burnblack-black">
                        ₹{capitalGainsAmount.toLocaleString()}
                      </span>
                    </div>
                  </div>
                );
      ...
      {totalForeignIncome > 0 && (
        <div className="dashboard-card-burnblack">
          {renderSectionHeader('Foreign Income Summary', Globe, 'foreignIncome', 
            Object.keys(filingData.income.foreignIncome || {}).length)}
          
          {expandedSections.foreignIncome && (
            <div className="px-4 pb-4 space-y-4">
              {Object.entries(filingData.income.foreignIncome || {}).map(([key, value]) => {
                if (!value || typeof value !== 'object') return null;
                ...
                switch (key) {
                  case 'salary':
                    foreignIncomeAmount = (value.grossSalary || 0) * (value.exchangeRate || 1);
                    incomeName = 'Foreign Salary';
                    break;
                  case 'business':
                    foreignIncomeAmount = (value.grossReceipts || 0) - (value.expenses || 0);
                    incomeName = 'Foreign Business';
                    break;
                  case 'other':
                    foreignIncomeAmount = (value.interest || 0) + (value.dividend || 0) + (value.royalty || 0) + (value.other || 0);
                    incomeName = 'Other Foreign Income';
                    break;
                  default:
                    return null;
                }
                
                if (foreignIncomeAmount === 0) return null;
                
                return (
                  <div key={key} className="p-3 bg-neutral-50 rounded-lg">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-neutral-600">{incomeName}</span>
                      <span className="text-sm font-semibold text-burnblack-black">
                        ₹{foreignIncomeAmount.toLocaleString()}
                      </span>
                    </div>
                    {value.country && (
                      <p className="text-xs text-neutral-500 mt-1">Country: {value.country}</p>
                    )}
                  </div>
                );
              })}
              ...
              <div className="border-t border-neutral-200 pt-3">
                <div className="flex justify-between">
                  <span className="text-lg font-semibold text-burnblack-black">Total Foreign Income</span>
                  <span className="text-xl font-bold text-burnblack-gold">
            ...
            {Object.entries(filingData.deductions || {}).map(([key, value]) => {
              ...
              switch (key) {
                ...
                case 'section80G':
                  ...
                  break;
                case 'section80E':
                  deductionAmount = value.educationLoanInterest || 0;
                  deductionName = 'Section 80E';
                  break;
                case 'section80TTA':
                  deductionAmount = Math.min(10000, value.savingsInterest || 0);
                  deductionName = 'Section 80TTA';
                  break;
                default:
                  return null;
              }
            ...
            {filingData.taxComputation && (
              <>
                ...
                {filingData.taxComputation.capitalGainsTax > 0 && (
                  <div className="p-3 bg-neutral-50 rounded-lg">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-neutral-600">Capital Gains Tax</span>
                      <span className="text-sm font-semibold text-burnblack-black">
                ...
                {filingData.taxComputation.foreignTaxCredit > 0 && (
                  <div className="p-3 bg-neutral-50 rounded-lg">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-neutral-600">Foreign Tax Credit</span>
                      <span className="text-sm font-semibold text-burnblack-black">
          ...
          <div className="px-4 pb-4 space-y-3">
            {validationResults.length === 0 ? (
              <div className="p-3 bg-success-50 rounded-lg text-center">
                ...
                <p className="text-xs text-success-600">Your ITR-2 is ready for filing</p>
              </div>
            ) : (
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className={`text-sm ${
              ...
              {isReadyForFiling 
                ? 'All validations passed. You can proceed to file your ITR-2.'
...
export default ITR2ReviewValidation;
